...
let set_max_var ?(size=7001) mv =
  max_var := mv;
  vt := Array.init mv (fun _ -> create size)
...
(* (* cache must reside outside of the function to reduce Gc overhead *)
let apply_cache = H2.create cache_default_size
*)
let gapply op =
  fun b1 b2 -> begin
    ...
    and app_gen ((u1,u2)(* as u12*)) =
      ...
      *)
        ...
        (*
        in
        H2.add apply_cache u12 res;
        res
        *)
    in
    (*
    H2.clear apply_cache;
    *)
    app (b1, b2)
  end
